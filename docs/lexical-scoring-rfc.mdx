...
### 1.1 Contaminated Nutrient Profiles

The previous mapping system used an 8-strategy cascade that fell back to raw `.includes()` substring matching when earlier strategies failed. This produced contaminated FDC memberships:

- **"oil"** (frequency: 9,841) mapped to 204 FDC foods, including every food containing the substring "oil" in descriptions like "b**oil**ed", "br**oil**ed", "par**oil**ed". The resulting nutrient profile showed ~4g fat instead of ~100g fat.
- **"butter"** mapped to "Cookies, butter, commercially prepared" via base-name bridge matching. The nutrient profile blended cookie macros with dairy fat.
- **"sugar"** mapped to "Cookies, sugar, refrigerated dough" via the same mechanism.

These are not data quality issues. In a medical nutrition API, a 25x error in fat content for the most common cooking oil is a **safety incident**.

### 1.2 Root Cause: Substring Matching Has No Word Boundaries
