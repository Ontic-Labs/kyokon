...
**Methodology:** [Empirical Ontology for High-Stakes Domains](/blog/empirical-ontology-pattern)
...
flowchart TD
    A["Recipe Corpus\n(231K recipes)"]
    B["recipe_ingredient_vocab\n(14,915)"]
    C["canonical_ingredient +\ncanonical_fdc_membership"]
    D["canonical_ingredient_nutrients\n(median, p10, p90, p25, p75, min, max)"]

    A -- "extract-recipe-ingredients.ts" --> B
    B -- "map-recipe-ingredients.ts" --> C
    C -- "aggregate-recipe-nutrients.ts" --> D
```
